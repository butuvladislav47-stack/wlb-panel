...
def client_active_count(client_id):
    return sum(1 for s in list_sessions() if s.get("client_id") == client_id and s.get("running"))
...
def start_session(name, client=None, managed=False):
    cs = cookie_status()
    if not cs["ok"]:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def api_client_start(self):
        ...
        with state_lock:
            session_id, link = start_session(session_name, client=client, managed=True)
...
<p>This permanent link is for a mobile WLB2 client app. The app calls the API below and receives a fresh temporary <code>wbstream://</code> link for this connection.</p>
...
<td>{status}<br><span class="muted small">active: {active}, limit: {esc(c.get('max_active') or 'unlimited')}</span></td>
