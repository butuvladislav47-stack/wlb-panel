...
CLIENT_IDLE_SECONDS = int(os.environ.get("WLB_CLIENT_IDLE_SECONDS", "90"))
...
def list_sessions():
    ...
    for d in sorted([x for x in SESSIONS_DIR.iterdir() if x.is_dir()], key=lambda p: p.stat().st_mtime, reverse=True):
        ...
        stopped = bool(st.get("stopped_at"))
        running = bool(not stopped and pid and creator_pid_alive(pid))
        out.append({
            "id": d.name,
            "pid": pid,
            "running": running,
...
def prune_stopped_sessions():
    ...
    for d in [x for x in SESSIONS_DIR.iterdir() if x.is_dir()]:
        ...
        pid = st.get("pid")
        link = read_text(d / "link.txt", "").strip()
        if st.get("stopped_at") or (pid and not creator_pid_alive(pid) and link.startswith("wbstream://")):
...
def cleanup_sessions():
    ...
    for s in list_sessions():
        if s.get("running") and s.get("managed") and s.get("role") != "reserve" and int(s.get("last_seen") or 0) < cutoff:
            stop_session(s["id"])
...
<p class="muted">Permanent client links survive reboot. Active rooms are created on demand; reserve rooms are hidden from the active list and counted in Clients.</p></div>
...
<div class="card"><h2>Active sessions</h2><p class="muted small">Reserve rooms are not shown here. Active client sessions without heartbeat are stopped after {CLIENT_IDLE_SECONDS}s.</p><table><thead><tr><th>Owner</th><th>Status</th><th>Temporary wbstream</th><th>Actions</th></tr></thead><tbody>{''.join(session_rows)}</tbody></table></div>
"""
