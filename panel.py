...
def find_chromium():
    for x in ["/usr/bin/google-chrome-stable", "/usr/bin/google-chrome", "/usr/bin/chromium", "/usr/bin/chromium-browser", "/snap/bin/chromium"]:
        if Path(x).exists():
            return x
    return None
...
def start_browser_stack():
    ...
    env["XDG_CONFIG_HOME"] = str(DATA_DIR / "xdg-config")
    env["XDG_CACHE_HOME"] = str(DATA_DIR / "xdg-cache")
    (DATA_DIR / "xdg-config").mkdir(parents=True, exist_ok=True)
    (DATA_DIR / "xdg-cache").mkdir(parents=True, exist_ok=True)
    ...
    x11vnc = subprocess.Popen(["x11vnc", "-display", display, "-rfbport", "5901", "-forever", "-shared", "-nopw", "-quiet", "-listen", "127.0.0.1"], stdout=log, stderr=log, env=env, start_new_session=True)
    time.sleep(1)

    chrome = find_chromium()
    if not chrome:
        raise RuntimeError("Браузер не найден. Установщик должен поставить google-chrome-stable")
    chrome_args = [
        chrome,
        "--no-sandbox",
        "--disable-gpu",
        ...
        "--start-maximized",
        "--app=https://stream.wb.ru/login",
    ]
    ch = subprocess.Popen(chrome_args, stdout=log, stderr=log, env=env, start_new_session=True)
    time.sleep(2)
    if ch.poll() is not None:
        raise RuntimeError("Браузер сразу завершился. Смотри /var/log/wlb-panel/browser.log")

    # websockify/noVNC запускаем после браузера, чтобы в iframe сразу была страница WB Stream, а не пустой рабочий стол.
