...
Server part for permanent WLB client links.
...
A future mobile app will save this permanent server link. On every connect, it
will call:
...
- when a mobile client connects again, the server creates a new temporary WB
  Stream room automatically.
...
2. call `start`;
3. connect internally to returned `join_link`;
4. send heartbeat every 30 seconds;
5. call `stop` when the user disconnects.
