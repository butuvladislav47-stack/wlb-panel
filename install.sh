...
if [[ -n "${CLI_ARCH}" ]]; then
  RELEASE_JSON="$(curl -fsSL --connect-timeout 10 --max-time 60 https://api.github.com/repos/kulikov0/whitelist-bypass/releases/latest || true)"
  ...
  if [[ -n "${CLI_URL}" ]]; then
    log "Downloading creator from latest upstream release..."
