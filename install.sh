...
apt-get update
apt-get install -y curl ca-certificates openssl python3 git golang-go unzip procps \
  xvfb x11vnc novnc websockify openbox || true
# Try Chromium names. Ubuntu may install snap chromium-browser.
...
chown -R root:wlbpanel "$CONFIG_DIR"
...
chown root:wlbpanel "$CONFIG_DIR/wb-cookies.json"
